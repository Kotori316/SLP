...
object NormInstanceL1 {
  ...
  implicit def pairNorm[A: Norm, B: Norm]: Norm[(A, B)] = (a: (A, B)) => a._1.norm + a._2.norm

  implicit def foldAbleNorm[A, F[_] : Foldable](implicit n: Norm[A], m: Monoid[A]): Norm[F[A]] = (a: F[A]) => a.foldLeft(0d) { case (a, b) => a + b.norm }
}
